...
export function shouldIgnore(filePath: string): boolean {
    ...
    if (lastDotIndex !== -1) {
        ...
        if (secondLastDot !== -1) {
            const compoundExtension = fileNameLower.substring(secondLastDot)
            if (IGNORED_EXTENSIONS.has(compoundExtension)) return true
        }
    }

    // ignore files that look like generated bundles or declaration files
    if (fileNameLower.includes('.bundle.') || fileNameLower.includes('.chunk.') || fileNameLower.includes('.generated.') || fileNameLower.includes('.d.ts')) return true
